...
pub fn check_composition(x: GraphWithComp) -> Result<(), IDCheckFail> {
    ...
    for f in graph.links.iter() {
        ...
        // where b = f.target; x = g.target
        for g in adj_matrix.get_all_morphs_from(f.target).into_iter().flatten() {
            // Morphism h: a -> x for all x
            // where a = f.source; x = g.target

            let fg = table.get_composition(&f, &g);
            let hs = adj_matrix.get_morphs(f.source, g.target);
            

        }


        // let hs = gs.map(|g| adj_matrix.get_morphs(g.target).into_iter().flatten()).flatten();
    



        let rhs = adj_matrix.get_all_morphs_from(f);
    }
